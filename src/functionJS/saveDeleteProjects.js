...
import {
    setPriority,
    buildProjectFooter,
} from '../displayJS/projectCardDisplay';
...
export function deleteProject(project) {
    ...
    currentStorage.forEach((element) => {
        if (element.title === project.title) {
            const index = currentStorage.indexOf(element);
            if (index > -1) {
                currentStorage.splice(index, 1);
            }
        }
...
export function changePriority(project) {
    ...
    currentStorage.forEach((element) => {
        if (element.title === project.title) {
            ...
            const priorDiv = document.getElementById(`${project.title + 5}`);
            const footDiv = document.getElementById(`${project.title + 55}`);
